...
interface CommitResultsListProps {
  rows: CommitRecord[];
  authorNamesById: string[];
}

export function CommitResultsList({ rows, authorNamesById }: CommitResultsListProps) {
  ...
  if (rows.length === 0) {
    return <div className="commit-empty-state">No commits match the current filters.</div>;
  }
    ...
    <div className="commit-results-viewport" ref={parentRef}>
      ...
      >
        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
            ...
            const authorName = authorNamesById[record.authorId] ?? 'Unknown';
